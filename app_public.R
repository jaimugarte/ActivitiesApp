...
ui =
  dashboardPagePlus(
    ...
    dashboardHeaderPlus(
      fixed = TRUE,
      title = tagList(span(class = "logo-lg", "Activities App")),
    ...
    dashboardBody(
      ...
      shiny::tags$head(
        shiny::tags$style(rel = "stylesheet",
                          type = "text/css",
                          href = "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/styles/qtcreator_dark.min.css"),
        shiny::tags$script(src = "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/highlight.min.js")
      ),
      tabItems(
        tabItem(tabName = "activities", fluidRow(box(
          uiOutput('activities'), width = 12
        ))),
        tabItem(tabName = "participants", fluidRow(box(
          uiOutput('participants'), width = 12
        ))),
        tabItem(tabName = "targets", fluidRow(box(
          uiOutput('targets'), width = 12
        ))),
        tabItem(tabName = "organizers", fluidRow(box(
          uiOutput('organizers'), width = 12
        ))),
        tabItem(tabName = "types", fluidRow(box(
          uiOutput('types'), width = 12
        ))),
        tabItem(tabName = "locations", fluidRow(box(
          uiOutput('locations'), width = 12
        ))),
        tabItem(tabName = "word1", fluidRow(
          box(
        ...
        tabItem(tabName = 'database', fluidRow(
          box(
            ...
            h5(
              "You can open the file dowloaded using this webpage: https://sqliteonline.com/"
            ),
            h5(
              "From this website you can view the tables of the database and download them in .csv format"
            ),
    ...
    footer = dashboardFooter(left_text = "By Jaime Ugarte",
...
server <- function(input, output, session) {
  ...
  instructorsR <-
    reactiveVal(getParticipants()$participant[getParticipants()$instructor == 'yes'])
  ...
  Activities <- dtedit(
    ...
    input.types = c(
      ...
      instructor = 'selectInputReactive',
    ...
    input.choices.reactive = list(
      organizers.list = organizersR,
      types.list = typesR,
      places.list = placesR,
      participants.list = participantsR,
      instructors.list = instructorsR,
  ...
  observeEvent(input$button, {
    ...
    instructorsR(getParticipants()$participant[getParticipants()$instructor == 'yes'])
  })
  ...
  dtedit(
    ...
    edit.cols = c('participant', 'instructor', 'phone', 'email', 'diet', 'notes'),
    edit.label.cols = c(
      'Participant',
      'Is he/she a instructor?',
      'Phone number',
      'Email',
      'Need special diet?',
      'Other comments'
    ...
    input.types = c(
      ...
      email = 'textInput',
      diet = 'textInput',
      instructor = 'selectInput',
      notes = 'textAreaInput'
    ),
    input.choices = list(instructor = c('no', 'yes')),
